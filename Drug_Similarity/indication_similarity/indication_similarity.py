...
def main():
    indicaions = pickle.load(open('./drug_indications_tfidf.pickle', 'rb'))
    features_matrix = []
    drug_list = []
    features_matrix = []
    for did in  indicaions.keys():
        drug_list.append(did)
        fp_list = list(map(int, indicaions.get(did)))
        # print(fp_list)
    ...
    print(similarity_df)
    similarity_df.to_csv("indicaions_similarity.csv")
